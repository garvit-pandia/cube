...
23. **Attempt lifecycle: `history` never resets itself** (`CubeController.afterTurnCompleted`, `scrambleCube`). `history` and `scramble` clear only on `reset()`; `applied` clears on `reset()` and whenever the cube reaches solved (manual or auto). The clock restarts only on a recorded turn while phase is `idle`/`ready` — never from `done` — and a session record requires phase `running` + solved + `history.length > 0`. Consequences: after `done`, press Scramble (→`ready`) before solving again or the solve won't record; auto-solve always lands in `done` without recording; `SolveRecord.moves` is `history.length`, so attempts after the first quote a cumulative count until Reset. Don't "fix" one half of this without the other.
